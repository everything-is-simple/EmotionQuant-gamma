...
- 与系统设计关系：若与 `docs/design-v2/rebuild-v0.01.md` 冲突，以设计 SoT 为准。
...
4. 记录文件不是可选项：每次任务都必须复核 `.kiro/record/*`。
...
- 明确目标、范围、输入输出、风险。
- 挂接路线图位置（`.kiro/roadmap/v0.01-mvp-roadmap.md` + 对应 `v0.01-mvp-spec-*.md`）。
- 开工前硬门控（强制）：
...
- 同步最小集合（强制）：
  1. `.kiro/record/development-status.md`
  2. `.kiro/record/debts.md`
  3. `.kiro/record/reusable-assets.md`
  4. `.kiro/roadmap/v0.01-mvp-roadmap.md`（阶段状态）
  5. 对应 `.kiro/roadmap/v0.01-mvp-spec-*.md`（任务勾选/进度）
...
1. `.kiro/record/development-status.md`
   ...
   - §5：追加 A6 收口一行（必选）
2. `.kiro/record/debts.md`：新增债务则登记；无则写 `debts=无变化`
3. `.kiro/record/reusable-assets.md`：新增资产则登记；无则写 `assets=无变化`
4. `.kiro/roadmap/v0.01-mvp-roadmap.md`：更新对应周次状态（TODO/IN_PROGRESS/DONE）
5. 对应 `.kiro/roadmap/v0.01-mvp-spec-*.md`：勾选本次完成项
