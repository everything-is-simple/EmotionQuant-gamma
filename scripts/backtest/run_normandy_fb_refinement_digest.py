...
from src.backtest.normandy_fb_refinement import build_normandy_fb_refinement_digest
...
def _resolve_latest_matrix_path(output_root: Path) -> Path:
    candidates = sorted(
        output_root.glob("*__fb_refinement_matrix.json"),
        key=lambda path: path.stat().st_mtime,
        reverse=True,
    )
    if not candidates:
        raise FileNotFoundError(f"No fb_refinement_matrix evidence found under {output_root}")
    return candidates[0]


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build Normandy FB refinement digest")
    parser.add_argument(
        "--matrix-path",
        default=None,
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Output JSON path; default normandy/03-execution/evidence/<run_id>__fb_refinement_digest.json",
    )
    return parser
...
def main() -> int:
    ...
    summary_run_id = build_run_id(
        scope="normandy_fb_refinement_digest",
        mode="dtt",
        variant=dtt_variant,
        start=start,
        end=end,
    ...
    output_path = (
        ...
        if args.output
        else output_root / build_artifact_name(summary_run_id, "fb_refinement_digest", "json")
    )

    payload = build_normandy_fb_refinement_digest(matrix_payload)
    payload["summary_run_id"] = summary_run_id
    path = write_normandy_volman_alpha_evidence(output_path, payload)
    print(f"normandy_fb_refinement_digest={path}")
    return 0
