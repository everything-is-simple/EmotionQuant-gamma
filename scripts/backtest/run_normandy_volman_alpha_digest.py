...
# Normandy / N1.5-C digest:
# 1. digest 既支持读取 scaffold matrix，也支持未来读取真实 Volman matrix。
# 2. 在 detector 未接通前，digest 只输出 pending 状态，不伪造 alpha 结论。
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build Normandy Volman second-alpha digest")
    parser.add_argument(
        "--matrix-path",
        default=None,
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Output JSON path; default normandy/03-execution/evidence/<run_id>__volman_alpha_digest.json",
    )
    return parser
...
def main() -> int:
    ...
    summary_run_id = build_run_id(
        scope="normandy_volman_alpha_digest",
        mode="dtt",
        variant=dtt_variant,
        start=start,
        end=end,
    ...
    output_path = (
        ...
        if args.output
        else output_root / build_artifact_name(summary_run_id, "volman_alpha_digest", "json")
    )

    payload = build_normandy_volman_alpha_digest(matrix_payload)
    payload["summary_run_id"] = summary_run_id
    path = write_normandy_volman_alpha_evidence(output_path, payload)
    print(f"normandy_volman_alpha_digest={path}")
    return 0
