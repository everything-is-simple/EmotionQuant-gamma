...
from src.backtest.positioning_null_control import (
    ...
    run_positioning_null_control_matrix,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Positioning P1 null control matrix")
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Output JSON path; default positioning/03-execution/evidence/<run_id>__null_control_matrix.json",
    )
    return parser
...
def main() -> int:
    ...
    working_db_path = (
        ...
        else cfg.resolved_temp_path / "backtest" / f"positioning-null-control-{date.today():%Y%m%d}.duckdb"
    )
    output_root = REPO_ROOT / "positioning" / "03-execution" / "evidence"
    scenarios = build_positioning_null_control_scenarios(cfg, initial_cash=initial_cash)
    variant_slug = sanitize_label("bof_control_no_irs_no_mss")
    summary_run_id = build_run_id(
        scope=POSITIONING_NULL_CONTROL_SCOPE,
        mode="dtt",
    ...
    output_path = (
        ...
        if args.output
        else output_root / build_artifact_name(summary_run_id, "null_control_matrix", "json")
    )

    payload = run_positioning_null_control_matrix(
        db_path=db_path,
        config=cfg,
        start=start,
        end=end,
    ...
    print(f"positioning_null_control_matrix={path}")
    return 0
