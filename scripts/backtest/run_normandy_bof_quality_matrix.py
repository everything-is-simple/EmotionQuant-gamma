...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Normandy N1.11 BOF quality matrix backtest")
    parser.add_argument("--start", required=True, help="Backtest start date (YYYY-MM-DD)")
    parser.add_argument("--end", required=True, help="Backtest end date (YYYY-MM-DD)")
    parser.add_argument("--cash", type=float, default=None, help="Initial cash override")
    parser.add_argument(
        "--dtt-variant",
        default=NORMANDY_BOF_QUALITY_DTT_VARIANT,
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Output JSON path; default normandy/03-execution/evidence/<run_id>__bof_quality_matrix.json",
    )
    return parser
...
def main() -> int:
    ...
    dtt_variant = sanitize_label(args.dtt_variant or NORMANDY_BOF_QUALITY_DTT_VARIANT)
    ...
    working_db_path = (
        ...
        else cfg.resolved_temp_path / "backtest" / f"normandy-bof-quality-{date.today():%Y%m%d}.duckdb"
    )
    output_root = REPO_ROOT / "normandy" / "03-execution" / "evidence"
    summary_run_id = build_run_id(
        scope="normandy_bof_quality_matrix",
        mode="dtt",
        variant=dtt_variant,
        start=start,
        end=end,
    ...
    output_path = (
        ...
        if args.output
        else output_root / build_artifact_name(summary_run_id, "bof_quality_matrix", "json")
    )

    payload = run_normandy_bof_quality_matrix(
        db_path=db_path,
        config=cfg,
        start=start,
        end=end,
    ...
    print(f"normandy_bof_quality_matrix={path}")
    return 0
