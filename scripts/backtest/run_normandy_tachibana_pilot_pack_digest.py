...
from src.backtest.normandy_tachibana_pilot_pack import (
    ...
    read_normandy_tachibana_pilot_pack_payload,
...
def _resolve_latest_matrix_path(output_root: Path) -> Path:
    candidates = sorted(
        output_root.glob("*__tachibana_pilot_pack_matrix.json"),
        key=lambda path: path.stat().st_mtime,
        reverse=True,
    )
    if not candidates:
        raise FileNotFoundError(f"No tachibana_pilot_pack_matrix evidence found under {output_root}")
    return candidates[0]


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build Normandy Tachibana pilot-pack digest")
    parser.add_argument(
        "--matrix-path",
        default=None,
        help="Matrix JSON path; default resolves the latest normandy/03-execution/evidence/*__tachibana_pilot_pack_matrix.json",
    )
    parser.add_argument(
        "--output",
        default=None,
        help="Output JSON path; default normandy/03-execution/evidence/<run_id>__tachibana_pilot_pack_digest.json",
    )
    return parser
...
def main() -> int:
    ...
    matrix_payload = read_normandy_tachibana_pilot_pack_payload(matrix_path)
    ...
    summary_run_id = build_run_id(
        scope=f"{NORMANDY_TACHIBANA_PILOT_PACK_SCOPE}_digest",
        mode="dtt",
        variant=variant,
        start=start,
        end=end,
    ...
    output_path = (
        ...
        if args.output
        else output_root / build_artifact_name(summary_run_id, "tachibana_pilot_pack_digest", "json")
    )
    ...
    print(f"normandy_tachibana_pilot_pack_digest={path}")
    return 0
