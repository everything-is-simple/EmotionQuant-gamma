...
from src.backtest.phase9_context_trend_direction_validation import (
    PHASE9_CONTEXT_TREND_DIRECTION_VALIDATION_SCOPE,
    run_phase9_context_trend_direction_validation,
    write_phase9_context_trend_direction_validation_evidence,
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Phase 9B context_trend_direction isolated validation")
    ...
    parser.add_argument(
        "--blocked-direction",
        default="DOWN",
        help="Context trend direction to block as the isolated negative guard",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Output JSON path; default docs/spec/v0.01-plus/evidence/"
            "<run_id>__phase9_context_trend_direction_validation.json"
        ),
    )
    return parser
...
def main() -> int:
    ...
    blocked_direction = str(args.blocked_direction).strip().upper()
    ...
    working_db_path = (
        ...
        else cfg.resolved_temp_path / "backtest" / f"phase9-context-direction-validation-{date.today():%Y%m%d}.duckdb"
    )
    output_root = REPO_ROOT / "docs" / "spec" / "v0.01-plus" / "evidence"
    variant_slug = sanitize_label(f"context_direction_negative_guard_{blocked_direction.lower()}")
    summary_run_id = build_run_id(
        scope=PHASE9_CONTEXT_TREND_DIRECTION_VALIDATION_SCOPE,
        mode="legacy",
    ...
    output_path = (
        ...
        / build_artifact_name(summary_run_id, "phase9_context_trend_direction_validation", "json")
    )

    payload = run_phase9_context_trend_direction_validation(
        db_path=db_path,
        config=cfg,
        start=start,
        end=end,
    ...
    path = write_phase9_context_trend_direction_validation_evidence(output_path, payload)
    print(f"phase9_context_trend_direction_validation={path}")
    return 0
