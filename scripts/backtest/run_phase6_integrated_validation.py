...
from src.backtest.phase6_integrated_validation import (
    PHASE6_INTEGRATED_VALIDATION_SCOPE,
    run_phase6_integrated_validation,
    write_phase6_integrated_validation_evidence,
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Phase 6B integrated end-to-end validation")
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Output JSON path; default docs/spec/v0.01-plus/evidence/<run_id>__integrated_validation.json",
    )
    return parser
...
def main() -> int:
    ...
    working_db_path = (
        ...
        else cfg.resolved_temp_path / "backtest" / f"phase6-integrated-validation-{date.today():%Y%m%d}.duckdb"
    )
    output_root = REPO_ROOT / "docs" / "spec" / "v0.01-plus" / "evidence"
    variant_slug = sanitize_label("phase6_unified_default_candidate")
    summary_run_id = build_run_id(
        scope=PHASE6_INTEGRATED_VALIDATION_SCOPE,
        mode="legacy",
    ...
    output_path = (
        ...
        if args.output
        else output_root / build_artifact_name(summary_run_id, "integrated_validation", "json")
    )

    payload = run_phase6_integrated_validation(
        db_path=db_path,
        config=cfg,
        start=start,
        end=end,
    ...
    path = write_phase6_integrated_validation_evidence(output_path, payload)
    print(f"phase6_integrated_validation={path}")
    return 0
