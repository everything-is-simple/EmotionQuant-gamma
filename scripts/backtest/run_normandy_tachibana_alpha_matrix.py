...
from src.backtest.normandy_tachibana_alpha import (
    NORMANDY_TACHIBANA_ALPHA_DTT_VARIANT,
    run_normandy_tachibana_alpha_matrix,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Normandy N1.8 Tachibana contrary alpha matrix backtest")
    parser.add_argument("--start", required=True, help="Backtest start date (YYYY-MM-DD)")
    parser.add_argument("--end", required=True, help="Backtest end date (YYYY-MM-DD)")
    parser.add_argument(
        "--dtt-variant",
        default=NORMANDY_TACHIBANA_ALPHA_DTT_VARIANT,
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Output JSON path; default normandy/03-execution/evidence/<run_id>__tachibana_alpha_matrix.json",
    )
    return parser
...
def main() -> int:
    ...
    dtt_variant = sanitize_label(args.dtt_variant or NORMANDY_TACHIBANA_ALPHA_DTT_VARIANT)
    ...
    working_db_path = (
        ...
        else cfg.resolved_temp_path / "backtest" / f"normandy-tachibana-alpha-matrix-{date.today():%Y%m%d}.duckdb"
    )
    output_root = REPO_ROOT / "normandy" / "03-execution" / "evidence"
    summary_run_id = build_run_id(
        scope="normandy_tachibana_alpha_matrix",
        mode="dtt",
        variant=dtt_variant,
        start=start,
        end=end,
    ...
    output_path = (
        ...
        if args.output
        else output_root / build_artifact_name(summary_run_id, "tachibana_alpha_matrix", "json")
    )

    payload = run_normandy_tachibana_alpha_matrix(
        db_path=db_path,
        config=cfg,
        start=start,
        end=end,
    ...
    print(f"normandy_tachibana_alpha_matrix={path}")
    return 0
