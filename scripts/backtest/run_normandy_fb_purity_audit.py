...
from src.backtest.normandy_fb_stability import (
    ...
    collect_normandy_fb_purity_snapshot,
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build Normandy FB purity audit")
    parser.add_argument(
        "--matrix-path",
        default=None,
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Output JSON path; default normandy/03-execution/evidence/<matrix_run_id>__fb_purity_audit.json",
    )
    return parser
...
def main() -> int:
    ...
    snapshot_payload = collect_normandy_fb_purity_snapshot(matrix_payload, db_path)
    payload = build_normandy_fb_purity_audit(matrix_payload, candidate_report_payload, snapshot_payload)

    matrix_variant = sanitize_label(str(matrix_payload.get("dtt_variant") or "unknown"))
    output_path = (
        Path(args.output).expanduser().resolve()
        if args.output
        else output_root / build_artifact_name(matrix_summary_run_id or matrix_variant, "fb_purity_audit", "json")
    )
    path = write_normandy_volman_alpha_evidence(output_path, payload)
    print(f"normandy_fb_purity_audit={path}")
    return 0
