...
from src.backtest.phase9_duration_percentile_validation import (
    PHASE9_DURATION_VALIDATION_SCOPE,
    run_phase9_duration_percentile_validation,
    write_phase9_duration_validation_evidence,
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Phase 9B duration_percentile isolated validation")
    ...
    parser.add_argument("--threshold", type=float, default=95.0, help="Duration percentile block threshold")
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Output JSON path; default docs/spec/v0.01-plus/evidence/<run_id>__phase9_duration_validation.json",
    )
    return parser
...
def main() -> int:
    ...
    working_db_path = (
        ...
        else cfg.resolved_temp_path / "backtest" / f"phase9-duration-validation-{date.today():%Y%m%d}.duckdb"
    )
    output_root = REPO_ROOT / "docs" / "spec" / "v0.01-plus" / "evidence"
    variant_slug = sanitize_label(f"duration_percentile_negative_filter_p{int(threshold)}")
    summary_run_id = build_run_id(
        scope=PHASE9_DURATION_VALIDATION_SCOPE,
        mode="legacy",
    ...
    output_path = (
        ...
        if args.output
        else output_root / build_artifact_name(summary_run_id, "phase9_duration_validation", "json")
    )

    payload = run_phase9_duration_percentile_validation(
        db_path=db_path,
        config=cfg,
        start=start,
        end=end,
    ...
    path = write_phase9_duration_validation_evidence(output_path, payload)
    print(f"phase9_duration_validation={path}")
    return 0
