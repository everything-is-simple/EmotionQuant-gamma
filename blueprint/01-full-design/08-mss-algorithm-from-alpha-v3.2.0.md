...
命名规范：情绪周期与趋势命名参见历史命名规范文档 §1-2（原链接已退役，现仅保留文本指引）。
...
**关联文档**：
- 数据模型：`mss-data-models.md`（历史来源文件，未迁入当前目录）
- API接口：`mss-api.md`（历史来源文件，未迁入当前目录）
- 信息流：`mss-information-flow.md`（历史来源文件，未迁入当前目录）
