...
命名规范：PAS机会等级与方向命名参见历史命名规范文档 §4-5（原链接已退役，现仅保留文本指引）。
...
**关联文档**：
- 数据模型：`pas-data-models.md`（历史来源文件，未迁入当前目录）
- API接口：`pas-api.md`（历史来源文件，未迁入当前目录）
- 信息流：`pas-information-flow.md`（历史来源文件，未迁入当前目录）
- 系统铁律：`Governance/steering/系统铁律.md`（历史来源文件，未迁入当前仓库）
