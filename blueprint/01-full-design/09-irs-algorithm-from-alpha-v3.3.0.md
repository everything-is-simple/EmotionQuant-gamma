...
命名规范：轮动状态命名参见历史命名规范文档 §3（原链接已退役，现仅保留文本指引）。
...
**关联文档**：
- 数据模型：`irs-data-models.md`（历史来源文件，未迁入当前目录）
- API接口：`irs-api.md`（历史来源文件，未迁入当前目录）
- 信息流：`irs-information-flow.md`（历史来源文件，未迁入当前目录）
