...
7. `l1_industry_member` 已不再卡死旧 `SW2021` 专用链，而是优先消费本地 generic industry bucket。
...
2. `cleaner.py` 与 `selector.py` 已切到 `l1_industry_member 优先 + l1_stock_info.industry 回退`。
3. `price_limit_rules` 已写回 `l1_stock_daily.up_limit / down_limit`，形成 `TDX local-first + local rules derived` 的真实执行底座。

当前仍未完成的尾项是：
1. 将 `l1_industry_member` 的表名与字段命名进一步从历史 `SW` 语义里剥离干净。
