...
def _build_digest_payload(
    *,
    ...
    return {
        ...
        "candidate_filter": {
            "metrics": {
                "duration_filter_total_signal_count": 20,
                "duration_filter_blocked_signal_count": blocked_signal_count,
                "duration_filter_blocked_signal_share": blocked_signal_count / 20.0,
                "duration_filter_rule": "block when current_wave_duration_percentile >= 95",
            }
        },
...
def test_duration_percentile_filter_blocks_only_threshold_hits_and_allows_missing_context() -> None:
    when = date(2026, 2, 24)
    signal_filter = DurationPercentileNegativeSignalFilter(threshold=95.0)
    store = _DummyStore(
        [
            {"code": "000001", "current_wave_duration_percentile": 96.0, "current_wave_duration_p95": 9.6},
            {"code": "000002", "current_wave_duration_percentile": 70.0, "current_wave_duration_p95": 9.6},
        ]
    )
    ...
    assert metrics["duration_filter_total_signal_count"] == 3
    assert metrics["duration_filter_blocked_signal_count"] == 1
    assert metrics["duration_filter_missing_percentile_signal_count"] == 1


def test_build_phase9_duration_validation_digest_promotes_when_candidate_cleanly_improves() -> None:
    digest = build_phase9_duration_validation_digest(
    ...
    assert digest["decision"] == "promote_duration_percentile_negative_filter"
    assert digest["diagnosis"] == "isolated_negative_filter_improves_baseline"


def test_build_phase9_duration_validation_digest_retains_sidecar_when_candidate_fails() -> None:
    digest = build_phase9_duration_validation_digest(
        _build_digest_payload(
