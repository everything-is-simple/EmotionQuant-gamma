...
from src.backtest.phase9_wave_role_validation import (
    ...
    WaveRoleNegativeSignalFilter,
    build_phase9_wave_role_candidate_label,
    build_phase9_wave_role_validation_digest,
)
...
def _build_digest_payload(
    *,
    ...
    candidate = {
        "scenario_label": PHASE9B_WAVE_ROLE_COUNTERTREND_NEGATIVE_FILTER,
        "window_label": "full_window",
        "trade_count": 9,
        "buy_filled_count": 9,
        "signals_count": 20,
    ...
    return {
        ...
        "candidate_filter": {
            "metrics": {
                "wave_role_filter_total_signal_count": 20,
                "wave_role_filter_blocked_signal_count": blocked_signal_count,
                "wave_role_filter_blocked_signal_share": blocked_signal_count / 20.0,
                "wave_role_filter_blocked_role": "COUNTERTREND",
                "wave_role_filter_rule": "block when current_wave_role == COUNTERTREND",
            }
        },
...
def test_wave_role_filter_blocks_countertrend_and_allows_missing_context() -> None:
    when = date(2026, 2, 24)
    signal_filter = WaveRoleNegativeSignalFilter(blocked_role="COUNTERTREND")
    store = _DummyStore(
        [
            {"code": "000001", "current_wave_role": "COUNTERTREND", "current_wave_role_basis": "CTX"},
            {"code": "000002", "current_wave_role": "MAINSTREAM", "current_wave_role_basis": "CTX"},
        ]
    )
    ...
    assert metrics["wave_role_filter_total_signal_count"] == 3
    assert metrics["wave_role_filter_blocked_signal_count"] == 1
    assert metrics["wave_role_filter_missing_role_signal_count"] == 1


def test_build_phase9_wave_role_validation_digest_promotes_when_candidate_cleanly_improves() -> None:
    digest = build_phase9_wave_role_validation_digest(
    ...
    assert digest["decision"] == "promote_wave_role_negative_filter"
    assert digest["diagnosis"] == "isolated_negative_filter_improves_baseline"


def test_build_phase9_wave_role_validation_digest_retains_sidecar_when_candidate_fails() -> None:
    digest = build_phase9_wave_role_validation_digest(
        _build_digest_payload(
...
def test_build_phase9_wave_role_candidate_label_reflects_blocked_role() -> None:
    assert build_phase9_wave_role_candidate_label("COUNTERTREND") == PHASE9B_WAVE_ROLE_COUNTERTREND_NEGATIVE_FILTER
    assert build_phase9_wave_role_candidate_label("mainstream") == "PHASE9B_WAVE_ROLE_MAINSTREAM_NEGATIVE_FILTER"
