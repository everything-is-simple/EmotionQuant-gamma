...
**当前系统口径**：
- A 股涨跌停实现边界见 `docs/steering/a-stock-rules.md`
- 当前模块落地见 `docs/design-v2/02-modules/selector-design.md`、`docs/design-v2/02-modules/broker-design.md`
- 当前主开发线见 `docs/spec/v0.01-plus/README.md`
**冲突处理**：若本文与当前系统设计冲突，以 `steering / design-v2 / spec` 当前版本为准；若与交易所官方规则冲突，以官方规则为准
**整理更新**：`2026-03-08`
...
1. 各板块涨跌停幅度与新股首日规则
2. ST、主板、科创板、创业板、北交所等外部制度差异
3. 规则校验、数据准备与参考计算方式

本文不直接回答：

1. 当前系统如何将涨跌停制度映射为情绪因子
2. 当前 IRS / MSS / PAS 如何消费这些外部规则
3. 当前 Broker 如何在执行层处理涨跌停边界

这些内容以当前系统设计和版本规格为准，而不是以本文旧集成表述为准。
