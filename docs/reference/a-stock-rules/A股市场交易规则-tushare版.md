...
**文档版本**：`v1.0`
**文档状态**：`Reference`
**适用范围**：A 股交易时间、撮合、停牌、T+1 与板块制度参考
**定位**：外部市场规则参考，不直接定义当前系统执行口径
**当前系统口径**：
- A 股硬约束与仓库内固定执行语义见 `docs/steering/a-stock-rules.md`
...
1. 交易时间与撮合制度
2. T+1、停牌、集合竞价等外部市场规则
3. 各交易所板块制度差异

本文不直接回答：

1. 当前系统 Broker 如何实现风控和撮合
2. 当前回测/纸上交易如何消费这些规则
3. 当前主开发线中 MSS、IRS、PAS 与这些规则的耦合方式

实现边界与执行细节以 `design-v2` 和 `spec` 当前版本为准。
...
- ✅ **仓库参考映射补充**: 可供 Broker、Backtest、Report 的外部规则校验参考
...
| 集合竞价 | 简单 | 详细时段划分 |
| T+1处理 | 基础 | 特殊场景覆盖 |
| 仓库参考映射（历史） | 交易规则校验 | 交易规则校验与执行边界参考 |
...
5. ✅ **仓库参考映射补充**: 支持当前 Broker / Backtest / Report 的规则校验参考
