...
**文档版本**：`v1.0`
**文档状态**：`Reference`
**适用范围**：A 股申万行业分类与 TuShare 取数参考
**定位**：外部规则与数据源参考，不直接定义当前系统执行口径
**当前系统口径**：
- `v0.01 Frozen` 历史基线见 `docs/design-v2/01-system/system-baseline.md`
- `v0.01-plus` 当前主开发线见 `docs/spec/v0.01-plus/README.md`
- 当前 IRS 执行/升级设计见 `blueprint/01-full-design/03-irs-lite-contract-annex-20260308.md` 与 `docs/spec/v0.01-plus/roadmap/v0.01-plus-spec-02-irs-upgrade.md`
...
1. 申万行业分类体系是什么
2. TuShare 如何返回相关分类与指数数据
3. 仓库实现 IRS 时可以参考哪些外部字段

本文不直接回答：

1. 当前 IRS 如何打分
2. 当前 IRS 是否包含轮动状态、牛股基因、相对量能
3. 当前主开发线如何消费行业数据

上述内容以当前设计文档为准，而不是以本文示例代码或旧集成表述为准。
...
为 IRS 侧的 `IndustryDailyData` 提供数据时，必须输出以下字段并保持命名/含义与权威一致（对齐 `blueprint/01-full-design/03-irs-lite-contract-annex-20260308.md` 与 `docs/design-v2/02-modules/data-layer-design.md`）：
