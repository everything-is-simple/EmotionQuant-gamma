...
**状态**: `Active`  
**封版日期**: `不适用（Active SoT）`  
**变更规则**: `允许按当前主开发线治理需要受控修订；若与系统设计口径冲突，优先修订上游设计与状态判定。`  
**上游文档**: `docs/spec/common/records/development-status.md`, `docs/spec/README.md`
...
1. 本文是当前仓库唯一固定执行流程 SoT。
2. 本文定义“任务如何推进、验证、归档、同步”。
3. 本文不定义系统算法正文，也不替代 `blueprint/`。
...
4. `docs/spec/<version>/roadmap/*.md`：更新对应阶段状态（TODO/IN_PROGRESS/DONE）
5. 对应 `docs/spec/<version>/roadmap/*spec*.md`：勾选本次完成项
...
| 日期 | 版本 | 变更 |
|---|---|---|
| 2026-03-08 | v1.3 | 改为版本无关工作流；当前主开发线默认挂接 `v0.01-plus`，历史问题按对应 `spec/<version>/` 归档 |
