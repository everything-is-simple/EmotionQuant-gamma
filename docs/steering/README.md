...
这里回答的是“哪些约束不能越界”，不是“完整系统设计细节写在哪里”。`v0.01 Frozen` 历史基线见 `docs/design-v2/01-system/system-baseline.md`；新版设计权威层见 `blueprint/README.md`；当前治理状态与重启条件以 `docs/spec/common/records/development-status.md` 为准。
...
1. `steering/` 是治理快照与快速约束层，不替代 `blueprint/` 的完整新版设计。
2. 若 `steering/` 与历史 `v0.01 Frozen` 基线冲突，以 `system-baseline.md` 为准；若与当前主线冲突，以 `blueprint/` 与 `docs/spec/v0.01-plus/` 为准，并应同步修订对应治理文档。
