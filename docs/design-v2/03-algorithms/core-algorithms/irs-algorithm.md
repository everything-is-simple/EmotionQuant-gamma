...
2. `G:\EmotionQuant\EmotionQuant-beta\docs\design\core-algorithms\irs\`
...
原始 `IRS-full` 中仍然成立、但当前执行版尚未完整恢复的部分包括：
...
这些字段在原始设计里不是“可有可无”的装饰，而是行业轮动系统成立所必需的解释层与决策层骨架。
...
因此它还称不上完整的 `Industry Rotation System`。
...
- 但它不应该长期停留在 IRS-lite；第一批真正有信息量的升级因子和轮动解释层已经明确提前纳入 `v0.01-plus`
