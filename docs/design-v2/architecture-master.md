...
- 改 PAS 形态 → 只改 `pas_bpb.py`，MSS/IRS 完全无感
...
| pas_bpb.py | `PatternDetector`(ABC) → `BpbDetector` | 多态：后续加 `PbDetector` 时统一接口 |
...
ACTIVE_PATTERNS = ["bpb"]        # 由 config.py 控制，新增形态只改这一行
...
│   │   ├── pas_bpb.py  # PAS BPB形态（第1迭代）
│   │   ├── pas_pb.py  # PAS PB形态（第2迭代）
│   │   ├── pas_tst.py  # PAS TST形态（第3迭代预留）
│   │   ├── pas_bof.py  # PAS BOF形态（第3迭代预留）
...
| 表名 | 用途 | 字段 | 加工逻辑 |
|------|------|------|----------|
| `l2_stock_adj_daily` | 个股复权日线 + 常用派生指标 | code, date, adj_open, adj_high, adj_low, adj_close, volume, amount, pct_chg, ma5, ma10, ma20, ma60, volume_ma5, volume_ma20, volume_ratio | 从 `l1_stock_daily` 的 adj_factor 算前复权价；pct_chg = (adj_close - prev_adj_close) / prev_adj_close；MA/量比用滚动窗口向量化计算 |
...
| `l3_signals` | PAS 每日信号历史 | pas_bpb.py | signal_id, code, signal_date, action, strength, pattern |
...
> 注：strong_up/down_ratio 和 panic/squeeze_tail_ratio 需在 L2 生成层逐日从 l2_stock_adj_daily 聚合计算，写入 l2_market_snapshot 对应字段。l1_stock_info 须包含 `market`（主板/创业板/科创板）和 `is_st` 字段用于分板块阈值计算。
...
**使用方式（第1迭代：硬过滤）**：selector.py 中，`gene_score > 阈值` 放行，衰股基因过重的直接排除。阈值默认 -30（宽松，只排除最差的）。后续迭代可改为软加权。
...
#### §4.3.1 pas_bpb（Breakout Pullback — 突破回踩）⭐ 第1迭代
...
#### §4.3.4 pas_bof（Breakout Failure — 假突破）第3迭代
...
|---------|-----------|---------|----------|
| **BPB** | FB + IRB杯柄 | FB=首次回调突破，杯柄=K线质量细节 | 第1迭代 |
| **PB** | FB（非首次）+ IRB | 简单回调=FB变种，边线反弹=IRB | 第2迭代 |
| **TST** | DD + SB | 双十字星=测试信号，二次突破=二次测试 | 第3迭代 |
| **BOF** | RB假突破 | 假突破识别=BOF入场 | 第3迭代 |
| **CPB** | SB + ARB | M/W形态=复杂回调，头肩底=高级形态 | 第3迭代 |

**核心增强技术**（应用于第1迭代pas_bpb）：
...
**应用优先级**：
- **第1迭代**：pas_bpb + IRB杯柄检测 + FB首次回撤 + 市场条件评估
- **第2迭代**：pas_pb + BB箱体增强（应用于所有形态）
- **第3迭代**：pas_tst/bof/cpb + DD/SB/RB/ARB结构 + 临界点动态管理
...
# ── PAS 形态配置
PAS_PATTERNS = ["bpb"]                # 当前活跃形态（第1迭代仅BPB），新增形态只改这一行
...
# 只跑 pb 形态的回测
python main.py backtest --patterns=pb --start=2023-01-01
...
第1迭代只有一个形态（bpb），默认 ANY。第 2 个形态加入时组合模式才生效。组合模式的选择直接影响左尾：ALL 模式信号更少但更严格，误触发少，左尾更薄。
...
| BACKUP（备选） | 严重失控 | 冷藏（信号跳过，不生成模拟单）；30 个交易日后自动回 OBSERVE |
...
**升级规则**：
- BACKUP：冷藏 30 个交易日后**自动**回到 OBSERVE（不直接回 ACTIVE）
...
| Data Layer L1-L4 四层架构 + 门禁 | 替换为 DuckDB 单库 | 偏"平台工程"，第1迭代不需要 |
...
> 四周计划 = 第1迭代落地。完成后进入第2迭代（pas_pb + 基因库 + IRS扩展因子）。
...
- 实现 pas_bpb.py（BPB形态检测 + Volman增强）
- 实现 strategy.py（信号汇总）
...
- 跑通第一次完整回测（单策略 pas_bpb + Volman增强）
