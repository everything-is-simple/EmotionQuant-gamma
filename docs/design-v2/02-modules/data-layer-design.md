...
**上游文档**: `docs/design-v2/01-system/architecture-master.md` §4.1  
**创建日期**: `2026-03-01`  
...
5. 首轮回测/重建优先复用本地 `raw_*` 历史库映射到执行库；`fetch` 主要负责增量补数，不把“重拉三年”当成默认主链路。
6. `amount` 字段默认采用 TuShare `千元` 口径；所有基于成交额的阈值必须显式标注单位，避免运行时误配。
7. `raw_stock_basic` 快照频率有限时，`l1_stock_info` 必须按 change-point 写入，并服务于下游 `as-of` 在市判断。
