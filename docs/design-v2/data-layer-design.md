...
class Store:
    ...
    def bulk_insert(self, table: str, df: pd.DataFrame) -> int:
        """
        批量 insert（无冲突场景，如 l3_signals 按 signal_id 唯一）。
        """
...
def clean_stock_adj_daily(store: Store, start: date, end: date):
    """L1 → l2_stock_adj_daily：前复权 + 均线 + 量比"""
...
步骤：
  ...
  2. 前复权：adj_open  = open  × adj_factor / latest_adj_factor
  ...
  6. 量比：volume_ratio = volume / volume_ma20
  7. bulk_upsert → l2_stock_adj_daily
```
...
strong_up_count       = 按板块分别计算（关键！不能统一用 ±5%）：
  主板:        pct_chg >= 0.05  (50% × 10%)
  创业板/科创板: pct_chg >= 0.10  (50% × 20%)
...
SELECT d.date,
       COUNT(*) FILTER (WHERE
           (i.market IN ('主板') AND NOT i.is_st AND d.pct_chg >= 0.05)
           OR (i.market IN ('创业板','科创板') AND NOT i.is_st AND d.pct_chg >= 0.10)
           OR (i.is_st AND d.pct_chg >= 0.025)
       ) AS strong_up_count
