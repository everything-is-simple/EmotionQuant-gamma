...
**状态**: `Frozen`  
**封版日期**: `2026-03-03`  
**变更规则**: `执行语义变更必须进入独立实验包或后续正式版本；算法权威入口、数据分类口径与链接修复允许在不改变执行语义前提下做受控纠偏。`  
**上游文档**: `无（系统总纲）`  
...
2. Selector：基础过滤 + MSS/IRS 漏斗，输出候选池，不输出买卖动作。
...
MSS/IRS 在 v0.01 视为待验证假设，但 `v0.01 Frozen` 的正式执行链路仍是 Selector 前置漏斗。回测/评审必须按以下顺序做消融对照：
...
- 现有 evidence 支持“`IRS + MSS soft overlay` 值得作为 `v0.01-plus` 主线替代方向”，但 `v0.01 Frozen` 本身仍保持前置漏斗口径不变。
- `v0.01-plus` 当前主开发线统一归档到 `docs/spec/v0.01-plus/`；当前设计入口见 `docs/design-v2/03-algorithms/core-algorithms/down-to-top-integration.md`，证据基础见 `docs/spec/v0.01/evidence/v0.01-evidence-review-20260306.md`。
...
沙盘推演与偏差闭环统一按 `docs/observatory/sandbox-review-standard.md` 执行（七维评审框架、证据模板、定稿门禁）。
