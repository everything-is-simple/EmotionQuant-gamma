...
> **版本**: v0.01 | **状态**: Active | **REV-ID**: `REV-v0.01-S05-20260304` | **基线**: `docs/design-v2/01-system/system-baseline.md` | **评审标准**: `docs/observatory/sandbox-review-standard.md`
...
**设计文档**: `docs/design-v2/02-modules/backtest-report-design.md`, `docs/design-v2/01-system/architecture-master.md` §4.5-4.6
...
- [ ] 输出诚信指标：`reject_rate/missing_rate/exposure_rate/failure_reason_breakdown`
- [ ] 输出机会参与指标：`opportunity_count/filled_count/skip_* / participation_rate`
...
- [ ] 幂等重跑以“主键集合一致”为硬验收：`order_id_set_equal=true`、`trade_id_set_equal=true`
...
8. 报告必须包含诚信指标与机会参与指标，不能只输出 EV/PF/MDD
9. 幂等验收必须证明 `l4_orders/l4_trades` 主键集合一致，而不是只比较汇总指标

## 校对证据（2026-03-04）

### 校对范围

1. `docs/design-v2/02-modules/backtest-report-design.md`
...
5. `docs/workflow/6A-WORKFLOW.md`
6. `README.md` / `README.en.md`

### 校对结论

1. 回测引擎边界一致：`backtrader` 仅时间推进/数据喂入，交易语义由自研 Broker 承担。
2. 报告口径一致：期望值、左尾、右尾、分环境中位数路径、消融对照均在 v0.01 范围内。
3. 审计链一致：`Signal -> Order -> Trade -> Report` 证据链未出现断点描述。

### 本轮修订记录

1. 新增本节“校对证据”，沉淀 backtest/report 的跨文档一致性结论。
2. 未改动 CLI、指标公式或预警阈值。
3. 基于基线跑通经验，将诚信指标、机会参与指标和主键集合幂等性提升为显式验收项。
