...
> **版本**: v0.01 | **状态**: Active | **REV-ID**: `REV-v0.01-S04-20260304` | **基线**: `docs/design-v2/01-system/system-baseline.md` | **评审标准**: `docs/observatory/sandbox-review-standard.md`

## 需求摘要
系统中唯一有"钱"的模块：接收信号 → 风控检查 → 下单 → 撮合成交。回测和纸上交易共用此内核。有状态（持仓、资金、信任分级）。

**设计文档**: `docs/design-v2/02-modules/broker-design.md`, `docs/design-v2/01-system/architecture-master.md` §4.4
...
### 现实执行约束（基线回望补强）
- `Order/Trade` 主键必须保持确定性：`order_id = signal_id`，`trade_id = f"{order_id}_T"`；重跑覆盖同键记录，禁止随机 ID 破坏幂等。
- Broker/Risk/Matcher 的全部时序判断必须依赖显式传入的 `trade_date / today / today_date`；回测链路禁止使用 `date.today()`。
- 信任分级与连续亏损统计只允许由已成交 `SELL` 交易驱动；`BUY` 成交不得触发 `update_trust()`，也不得稀释连亏计数。
- `paper` 持仓不是“旁路仓位”：仍必须经过止损、止盈、组合回撤检查，并生成 `paper SELL` 保持 OBSERVE→ACTIVE 路径可达。
...
- [ ] `execute_orders`: 遍历 Order → matcher.execute → 更新 portfolio/cash；仅 `SELL` 成交允许触发 `update_trust`
...
8. 同一组输入重跑后，`order_id` / `trade_id` 集合完全一致，不出现随机主键漂移
9. `BUY` 成交不会更新信任状态，也不会影响连续亏损熔断统计；仅 `SELL` 参与该链路
10. `paper` 持仓同样执行退出规则，并能走通 OBSERVE → ACTIVE（试用）→ BACKUP 的信任状态机
...
3. `docs/observatory/sandbox-review-standard.md`
4. `docs/steering/a-stock-rules.md`
5. `docs/steering/architecture.md`
...
3. 基于基线跑通经验，补充确定性主键、显式回测日期、SELL-only trust update 与 paper 持仓退出约束。
