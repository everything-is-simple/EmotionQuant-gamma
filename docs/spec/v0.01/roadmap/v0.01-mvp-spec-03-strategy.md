...
> **版本**: v0.01 | **状态**: Active | **REV-ID**: `REV-v0.01-S03-20260304` | **基线**: `docs/design-v2/01-system/system-baseline.md` | **评审标准**: `docs/observatory/sandbox-review-standard.md`

## 需求摘要
对候选池中每只股票判断买卖时机。PAS 形态检测器架构：每个形态一个检测器，签名统一，可独立回测，config 驱动装配。v0.01 只实现 BOF（假突破反杀）并单形态跑通。

**设计文档**: `docs/design-v2/02-modules/strategy-design.md`, `docs/Strategy/PAS/volman-ytc-mapping.md`, `docs/design-v2/01-system/architecture-master.md` §4.3
...
### pattern_base.py
- [x] 定义 PatternDetector ABC（name + detect 抽象方法）
- [x] 定义 df 输入规范注释（列要求 + 行排序要求）

### pas_bof.py
- [x] 实现 BofDetector 类（name="bof"，含 config 参数）
- [x] Spring 触发判定（4条件全满足即产出 BUY Signal，无延续确认期）
- [x] strength 计算（收盘位置 + 量比 + 反转K线实体占比）
- [x] 边界保护（窗口不足20天 / 分母为零 / 一字涨跌停 → return None）
- [x] 单测：构造假破位数据，验证触发/不触发/边界保护
- 注：失效退出（次日不延续/跌回结构）由 Broker 风控层执行，不在 detector 内实现

### registry.py
- [ ] 实现 ALL_DETECTORS（五形态在册，v0.01 仅启用 bof）
- [x] 实现 `get_active_detectors(config)`（按 PAS_PATTERNS 过滤+实例化）
- [x] 预留第2/3迭代检测器注释位
- [x] v0.01 验收时强校验：`len(PAS_PATTERNS) == 1 and PAS_PATTERNS[0] == \"bof\"`

### strategy.py
- [x] 实现 `generate_signals`（遍历候选 × 检测器）
- [x] 实现 `_combine_signals`（ANY/ALL/VOTE 三种模式）
- [x] 写入 l3_signals（**bulk_upsert**，signal_id 确定性幂等键，重跑覆盖而非追加，D7/H17控制点）
- [x] 单测：mock 多信号输入，验证三种组合模式

### config 配置
- [x] PAS_PATTERNS = ["bof"]
- [x] PAS_COMBINATION = "ANY"
- [x] PAS_LOOKBACK_DAYS = 60
- [x] PAS_MIN_HISTORY_DAYS = 30
- [x] PAS_BOF_BREAK_PCT = 0.01
- [x] PAS_BOF_VOLUME_MULT = 1.2
...
4. `docs/spec/v0.01/roadmap/v0.01-mvp-roadmap.md`
5. `docs/steering/product.md`
6. `docs/steering/architecture.md`
7. `AGENTS.md` / `CLAUDE.md` / `WARP.md`

### 校对结论

1. v0.01 仅 BOF 触发器在线，其他形态在册但不启用，口径一致。
2. 执行语义一致：`signal_date=T -> execute_date=T+1 -> T+1 Open`，未发现 Strategy 侧漂移。
3. Strategy 边界一致：只产 BUY，SELL 由 Broker 风控生成。

### 本轮修订记录

1. 新增本节“校对证据”，补齐文档级审计留痕。
2. 未修改 BOF 条件、组合模式默认值与执行语义。
3. 2026-03-06 补回实现进度勾选：BOF 检测器、信号合并、幂等写入、`PAS_MIN_HISTORY_DAYS` 与单形态校验已落地。

## 已知风险与偏差

| 级别 | 问题 | Owner | 截止日期 | 状态 |
|------|------|-------|----------|------|
| S3 | `ALL_DETECTORS` 仍未以占位 detector 形式补齐 `bpb/tst/pb/cpb`，当前仅保留注释位 | Codex | v0.02 启动前 | open |
