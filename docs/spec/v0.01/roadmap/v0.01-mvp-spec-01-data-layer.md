# Spec 01: Data Layer

> **版本**: v0.01 | **状态**: Active | **REV-ID**: `REV-v0.01-S01-20260304` | **基线**: `docs/design-v2/01-system/system-baseline.md` | **评审标准**: `docs/observatory/sandbox-review-standard.md`

## 需求摘要
搭建系统地基：从 TuShare 拉取 A 股数据、清洗加工、存入 DuckDB、定义模块间契约。零业务逻辑。

v0.01 的现实执行路径已明确：

1. **本地历史库优先**：优先复用本地 `raw_*` 历史库映射到执行库，不把“重拉三年”当成默认主链路。
2. **增量抓取兜底**：`fetch` 负责增量更新、断点续传和备源兜底，不假设每次都能从 TuShare 全量重建。
3. **单位必须写清楚**：`amount` 口径来自 TuShare `千元`，所有流动性阈值必须显式注明单位。

**设计文档**: `docs/design-v2/02-modules/data-layer-design.md`, `docs/design-v2/01-system/architecture-master.md` §4.1

## 交付文件

| 文件 | 职责 |
|------|------|
| `src/config.py` | 全局配置（环境变量、开关、常量） |
| `src/contracts.py` | pydantic 模块边界契约（6个类） |
| `src/data/store.py` | DuckDB 统一存取层（建表、upsert、查询） |
| `src/data/fetcher.py` | L1 数据下载（TuShare主 + AKShare备） |
| `src/data/cleaner.py` | L1→L2 清洗加工（复权、均线、聚合） |
| `src/data/builder.py` | L2/L3 增量生成调度（L4 由 broker/report 运行时写入，不经过 builder） |

## 设计要点

### store.py
- Store 类持有唯一 DuckDB 连接，启用 WAL
- `bulk_upsert(table, df)`: 注册 df 为临时视图 → INSERT OR REPLACE
- `read_df(sql, params)`: 统一查询入口
- `get_fetch_progress` / `update_fetch_progress`: 断点续传元数据
- main.py 创建 Store 实例，参数传递给各模块（不用全局单例，测试时注入 `:memory:`）

### fetcher.py
- DataFetcher(ABC) → TuShareFetcher / AKShareFetcher 多态
- `fetch_stock_daily`: 按月分批，daily() + daily_basic() 按 (ts_code, date) 合并
- 降级粒度为整次运行（不在单次 API 调用级别切换）
- 多线程并行：4种数据类型（stock_daily/index_daily/stock_info/trade_cal）并行拉取，主线程顺序写入
- tenacity 重试 3 次，API 调用间 sleep 0.3s
- 现实链路：优先复用本地 `raw_*` 历史库，`fetch` 主要承担增量补数与断点续传

### cleaner.py
- `clean_stock_adj_daily`: 后复权(adj = raw × adj_factor) → pct_chg → MA5/10/20/60 → 量比
- `clean_market_snapshot`: 全市场截面统计（strong_up/down 按板块分别计算阈值！）
- `clean_industry_daily`: 按申万一级聚合（等权平均涨跌幅）
- 增量生成时从 start-60 天读 L1（滚动窗口），只写 [start, end] 到 L2
- `l1_stock_info` 必须按 `effective_from` 做 change-point 写入，处理 `raw_stock_basic` 快照稀疏性

### contracts.py
- 6 个 pydantic BaseModel: MarketScore, IndustryScore, StockCandidate, Signal, Order, Trade
- 关键 id 字段采用确定性规则（见 `docs/steering/conventions.md`）：`signal_id = f"{code}_{signal_date}_{pattern}"`，`order_id = signal_id`，`trade_id = f"{order_id}_T"`

## 实现任务

### 基础框架
- [ ] 创建 `src/` 目录结构和 `__init__.py`
- [ ] 实现 `src/config.py`（DATA_PATH、TuShare token、全部开关和常量）
- [ ] 实现 `src/contracts.py`（6 个契约类）
- [ ] 更新 `pyproject.toml` 依赖

### store.py
- [ ] 实现 Store.__init__（连接 DuckDB、启用 WAL、执行全部 DDL）
- [ ] 实现全部 L1-L4 建表 DDL（含 _meta_fetch_progress, _meta_runs）
- [ ] 实现 bulk_upsert / bulk_insert / read_df / read_scalar / read_table
  - `read_scalar(sql, params)` 返回单个标量值（仓位计算等场景需要，K29控制点）
- [ ] 实现 get_fetch_progress / update_fetch_progress / get_max_date
- [ ] 单测：`:memory:` 库上验证 CRUD

### fetcher.py
- [ ] 实现 DataFetcher ABC（4 个抽象方法）
- [ ] 实现 TuShareFetcher（fetch_stock_daily 含 daily+daily_basic 合并）
- [ ] 实现 AKShareFetcher 骨架（字段映射到 TuShare 格式）
- [ ] 实现 create_fetcher 工厂函数（探活 → 降级）
- [ ] 实现 fetch_incremental（断点续传）
- [ ] 实现多线程并行拉取（ThreadPoolExecutor + 队列 + 主线程顺序写入）
- [ ] 单测：mock API 返回，验证合并逻辑和断点续传

### cleaner.py
- [ ] 实现 clean_stock_adj_daily（后复权 adj=raw×adj_factor + pct_chg + MA + 量比，向量化）
- [ ] 实现 clean_market_snapshot（17 个字段，strong_up/down 按板块 JOIN l1_stock_info）
- [ ] 实现 clean_industry_daily（按申万一级聚合）
- [ ] 实现 ts_code_to_code 转换
- [ ] 单测：构造 mock L1 数据，验证 L2 输出正确性

### builder.py
- [ ] 实现 CLI 接口（--layers, --start, --end, --force）
- [ ] 实现增量生成逻辑（检查 max(date)，只算新日期）
- [ ] 实现强制全量重建（--force）

### 集成验证
- [ ] 端到端：fetch → clean → build，拉取近 3 年数据写入 DuckDB
- [ ] 端到端：优先验证“本地历史库映射 -> clean -> build -> 增量补数”链路
- [ ] 验证 L1 表行数合理（~5000股 × ~750天 ≈ 375万行）
- [ ] 验证 L2 表 MA/量比无异常 NaN（窗口不足的首 60 天除外）
- [ ] 验证每日仅增量更新（禁止全量重拉）
- [ ] 验证 API 限流与断点续传生效（失败后可继续）
- [ ] 验证 `amount` 阈值按“千元”口径配置，不出现单位错配导致的候选池误清空

## 验收标准
1. `python main.py fetch` 能完成增量更新；当本地 `raw_*` 历史库已存在时，不要求重复全量拉取 3 年数据
2. `python main.py build --layers=l2` 能生成全部 L2 表
3. L2 表的 strong_up_count 按板块分别计算（不是统一 ±5%）
4. contracts.py 全部类型可实例化和序列化
5. Store 单测在 `:memory:` 库上通过
6. `l1_stock_info` 能支撑 `as-of` 在市状态判断，不因快照稀疏导致信号级幸存者偏差

## 校对证据（2026-03-04）

### 校对范围

1. `docs/design-v2/02-modules/data-layer-design.md`
2. `docs/design-v2/01-system/system-baseline.md`
3. `docs/spec/v0.01/roadmap/v0.01-mvp-roadmap.md`
4. `docs/steering/architecture.md`
5. `docs/steering/conventions.md`
6. `docs/spec/common/records/development-status.md`
7. `AGENTS.md` / `CLAUDE.md` / `WARP.md` / `README.md`

### 校对结论

1. v0.01 Data 层口径与 SoT 一致：L1/L2/L3/L4 分层、T+1 语义、`ts_code -> code` 转换边界一致。
2. 本卡与路线图一致：Week 1 目标、交付文件、验收口径未出现跨版本漂移。
3. 与治理文件一致：仍满足“先校对后实现”的 6A 流程要求。

### 本轮修订记录

1. 补充本节“校对证据”作为 spec-01 文档级可追溯证据落点。
2. 明确本轮校对范围、结论与修订动作，避免只给结论不给证据。
3. 基于基线跑通经验，补充“本地历史库优先、增量抓取兜底、amount 单位、快照稀疏性”这四项现实约束。

## 已知风险与偏差

| 级别 | 问题 | Owner | 截止日期 | 状态 |
|------|------|-------|----------|------|
| — | 当前无已知 S2+ 偏差 | — | — | — |




