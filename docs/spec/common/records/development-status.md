...
| 2026-03-06 | 实现风险 | Week2 审计显示：MSS 全区间 `768/768` 为 `NEUTRAL`；IRS 当前使用 `111` 行业桶；全周期三场消融超过 `75` 分钟未收口 | 保留短窗证据继续推进；Week2 不封单，后续优先处理 MSS baseline 校准、IRS 行业桶口径决策与 ablation 性能问题 | open |
